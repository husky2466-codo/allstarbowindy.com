...
> **▶ START HERE — what just shipped:** the **Tier-A member area** (`#/account` signed-in side) is
> **BUILT + verified** (see the entry directly below). Open items it surfaced for the *owner*: confirm
> member scale/audience before the Cloudflare Access gate ships; the captured-games + bag data are demo
> stubs in `js/data-provider.js` awaiting the real D1 feed (see `docs/DATA-CONTRACT.md` §2a). Next
> candidates from the punch list: Cafe real prices + Alley Lounge page ·
> Specials extras · Phone-case kiosk · Interactive Bowling.

**PARTIES full pricing tiers — built (latest, this session):** Completed the Parties page (`js/parties.jsx`,
styles `.ptier-*` / `.pseason-*` in `pages3.css`) with the real group/event pricing from the content brief
§5. Two new sections after the birthday estimator: **`PartyTiers`** — "Group party packages" (Weekday
$10.50/$14 · Primetime $13/$17.75 per guest as 2-/3-game rows, featured "Most popular" Primetime card, a navy
**Food add-on** card $12/$9-kids) + a notes row (seats 67 · 48 lanes · shoes incl · full bar · 50% deposit) +
a Corporate/team-building call-to-book strip; and **`PartySeasonal`** (navy field) — NYE Family (~$80/lane),
NYE Adult Midnight Casino Bowl ($25 Cosmic / $30 Interactive per person), Junior Gold Practice ($120 / 8-wk).
All static real content (no data layer needed). Framed so birthday (all-inclusive, above) vs. group (bowling +
optional food) is clear. Verified prices match the brief; no console errors.

**ACCOUNT ENTRY + LOGIN FLOW — built (latest, this session):** Gave the member area a real way in
(it was only reachable via an in-page preview toggle before) and the sign-in UX the docs never specified
(the *auth model* — Cloudflare Access, Google / email-OTP, no password — was documented; the nav placement
+ click flow were not).
- **Persistent demo session** — new `AuthStore` + `useAuth()` in `components.jsx` (localStorage key
  `asb_auth`), a subscribe/notify store like `ClockStore`. It's just a signed-in *flag*; member DATA still
  comes from `window.ASB_DATA`. Survives navigation + refresh. In production this flag = "Cloudflare Access
  authenticated"; swap `AuthStore`/the login page for the Access redirect, member data layer unchanged.
- **Account menu in the nav** — new `AccountMenu` (in `components.jsx`, far-right of `.nav-right`): a person
  icon (logged-out) → a colored circle with the member initial (logged-in). Dropdown swaps by state —
  **logged-out:** Sign in · Join the club (free) → `/join` · Rewards & perks → `/account` · Find my USBC
  average (bowl.com link-out) · League standings → `/scores`; **logged-in:** name/email header · My Stats ·
  My Bag · Scores & standings · Account settings · Sign out. Deep links fire a `window` `asb-matab` event
  (+ set `window.__maInitialTab`) so they switch the member-area tab even when already on `/account`.
  Styles `.acctmenu*` in `components.css`. Mobile sheet got an **Account** group too.
- **Nav recluster** (user's pick): right side is now **Pro Shop · phone · Reserve a Lane | cart · account**
  (a `.nav-div` separates the two clusters). ⚠️ `.nav-phone` is hidden ≤1300px by a pre-existing
  `scores.css` rule (nav is tight; the account icon adds width) — phone shows only on wide desktop, else it's
  in the mobile sheet. Order is correct when visible.
- **Full branded login page** (`#/login`, new `js/login.jsx` + `assets/login.css`, route in `app.jsx`):
  split layout — mural brand panel (left) + sign-in card (right). **No password** (honors the Access model):
  *Continue with Google* (real Google "G" mark) **or** email → **6-box one-time-code** step (any 6 digits in
  the demo) → sign in. "Secured by Cloudflare Access" fine print; "Join free" link; already-signed-in shows a
  shortcut instead. `LoginPage` reads `useAuth()`. **Graffiti-styled (latest):** the member zone is
  deliberately *not* the cream/paper of public pages — the form panel + already-signed-in screen sit on a
  bespoke **navy graffiti wall** (`assets/img/login-graffiti-bg.png`, generated in `run_script` — paint dots,
  drips, stars, calmer center band), brand panel keeps the real mural, white card stays clean with a sprayed
  red underline + spray divider. The bg is a **swap slot** — a hand-painted/AI upgrade dropped at that path
  replaces it with no code change (prompt in `Hero Image Prompts.md` §5; same for the wipe confetti).
- **Corner Buddy greetings extended for the new nav** (`mascot.jsx` `BUDDY_ROUTE_LINES` / `buddyLineFor`):
  added `/login` ("Roll on in!") and made `/account` **auth-aware** — signed-in members get
  "Welcome back, hot shot!" (`BUDDY_ACCOUNT_IN`, gated on `AuthStore.isAuthed()`) instead of the logged-out
  "Earn free games!" pitch.
- **Bowling-ball wipe transition** — new global `<BallWipe/>` + `WipeStore.play(onCovered)` (mounted in
  `app.jsx` beside `CornerBuddy`). rAF-driven (CSS anims are frozen in preview): a navy/blue curtain (red
  diagonal stripes + halftone) sweeps across led by a rolling, rotating bowling ball; at full cover
  (`p≥0.5`) it fires `onCovered` — which flips `AuthStore.signIn()` and routes to `/account` — then sweeps
  off to reveal the member area. "Rolling you in…" word, opacity tied to coverage. Respects
  `prefers-reduced-motion` (skips straight to the callback). Reusable for any "wipe then navigate" moment.
  **Curtain art = a bespoke generated graffiti confetti texture** (`assets/img/wipe-confetti.png`, made in
  `run_script` with canvas — multicolor paint dots + stars + rings in the brand palette, echoing the real
  mural's dot/star halo) over the navy gradient, with a slight parallax drift (replaced the earlier diagonal
  pinstripes per the user).
- **`AccountPage` rewired** (`account.jsx`): dropped the old `Preview: Not a member / Signed-in member`
  toggle bar; now `useAuth()` decides — authed → `<MemberArea/>` (sign out = `AuthStore.signOut()` → home),
  else → `<RewardsTeaserPage/>` ("I'm already a member" / "Join free" → `/login`).
- **New member-area Settings tab** (`MemberSettings` in `member-area.jsx`): Profile (name/email/ID from
  "Access", read-only) + USBC look-up (optional ID, "we don't store it") + a labeled **Phase 2** card for
  notifications / data export-delete. MA tabs are now **My Stats · Equipment · My Bag · Scores & More ·
  Settings**.
- **Verified** (DOM rects + flow eval, per the capture-quirk rule): account menu (both states), `/login`
  render, Google + code paths, the wipe (captured mid-roll), landing authed on `/account`, persistence across
  a real reload, deep-link tab switching, Settings render, sign-out → teaser. No console errors (only the
  Babel warning). Demo ships **logged-out** so the sign-in flow is the first thing you can try.

**Tier-A MEMBER AREA — built (this session):** Rebuilt the `#/account` signed-in side against
`docs/MEMBER-AREA-SCOPING.md` + `MEMBER-METRICS-SUITE.md`. **Tier-A only, zero fabricated data, all read
through the `window.ASB_DATA` seam.** The old fabricated Marcus-Bell/Gold/164 `Dashboard` + `DashboardEmpty`
were deleted from `account.jsx`.
- **New `js/member-area.jsx` + `assets/member.css`** (loaded before `account.jsx` / after `pages3.css`).
  `AccountPage`'s signed-in branch now renders **`<MemberArea/>`** — a tabbed app surface (light "dashboard"
  skin on the venue brand). Exports via `Object.assign(window,…)`.
- **Tabs:** **My Stats** · **Equipment** · **My Bag** · **Scores & More**.
  - *My Stats* — metrics **DERIVED** from captured games in `computeMetrics()` (average, high game, high
    3-game series, count, last-5 form), a **static SVG average-trend chart** (`TrendChart`: per-game line +
    running-average line, no CSS animation), and a session log (grouped by date, expandable). **Empty state**
    ("No games tracked yet") when `games:[]` — the honest launch reality; never a fake number.
  - *Equipment* — the **183-ball spec catalog** (`window.ASB_BALLS`, new `js/ball-catalog.js`, imported from
    repo `reference/bowling-catalog/BallDatabase.json`, compacted one-ball-per-line). Search + brand select +
    cover/core chips + sort; gradient `BallSwatch` art (no images); click → `BallDetail` modal (full specs +
    plain-language explainers + "add to My Bag" + a *Go to Pro Shop* cross-link). Catalogs kept **separate**
    per the user: this is the spec/reference browser, Pro Shop stays the sales/reserve experience.
  - *My Bag* — member-tagged balls via `ASB_DATA.getBag/addToBag/removeFromBag/inBag`, **localStorage-backed**
    (`asb_member_bag`, seeded with 4) so saves survive refresh; own empty state; *Reserve at the Pro Shop* CTA.
  - *Scores & More* — real link-outs (Live Scores `#/scores`; standings → Computer Score centre 112; **Find my
    USBC average** → bowl.com with name in the query, "we never store it"); static **BAC tier explainer**
    (general program info, not personal status); a clearly **labeled "Phase 2 — pending data"** card for the
    Tier-B personal tier / visit history.
- **Demo toggle** in the member header (`Demo data: On / Empty state`) flips `member.games` to `[]` to preview
  the empty state live — derived from the real member object, not a second fabricated one. Populated demo is the
  default (per the user).
- **Data seam updated** (`js/data-provider.js`): `member` now carries a **~63-game captured season**
  (`{date,score,lane,league,leagueName}`, gentle upward trend, avg ~177 / high 268 / high series 646) + profile
  (incl. email from "Access") + `tierKey` (Tier-B placeholder); added the four **bag** accessors. `DATA-CONTRACT.md`
  §1/§2a/§2a-bis/§4 updated to the new shape + `window.ASB_BALLS`.
- **Verified** via DOM rects (per the capture-quirk rule): all four tabs render, metrics derive correctly,
  trend SVG draws, empty-state toggle works, detail modal opens, add/remove bag works, bag badge counts. No
  console errors (only the expected Babel-in-browser warning). ⚠️ html-to-image **captures mis-render the
  condensed display font** on the Equipment filter row (looks overlapped) — DOM rects confirm the real layout
  is clean; trust the rects.
...
12. ✅ **DONE (this session)** — **Parties full pricing tiers** — Weekday ($10.50/$14), Primetime ($13/$17.75),
    food add-on ($12/$9 kids), 50% deposit, NYE Family (~$80/lane) + NYE Adult ($25 cosmic / $30 interactive),
    Junior Gold ($120 / 8 wk). Built as `PartyTiers` + `PartySeasonal` on `#/parties`.
